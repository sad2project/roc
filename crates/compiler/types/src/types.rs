...
struct AliasShared {
    symbol: Symbol,
    type_argument_abilities: Slice<Option<Symbol>>,
    type_argument_regions: Slice<Region>,
    lambda_set_variables: Slice<LambdaSet>,
}
...
struct Types {
    ...
    // aliases
    ability_symbols: Vec<Option<Symbol>>,
...
impl Types {
    ...
    fn alias_shared_help(
        ...
        let type_argument_abilities = Slice::extend_new(
            &mut self.ability_symbols,
            type_arguments.iter().map(|a| a.opt_ability),
        );
    ...
    fn from_old_type_at(&mut self, index: Index<TypeTag>, old: &Type) {
        match old {
            ...
            Type::DelayedAlias(alias_common) => {
                //    pub symbol: Symbol,
                //    pub type_arguments: Vec<Loc<OptAbleType>>,
                //    pub lambda_set_variables: Vec<LambdaSet>,
                ...
                let type_argument_regions = Slice::extend_new(
                    &mut self.regions,
                    alias_common.type_arguments.iter().map(|t| t.region),
                );

                let type_arguments_slice = {
                    let slice = self.reserve_type_tags(alias_common.type_arguments.len());

                    for (index, argument) in slice.into_iter().zip(&alias_common.type_arguments) {
                ...
                let lambda_set_slice = {
                    let slice = self.reserve_type_tags(alias_common.lambda_set_variables.len());

                    let it = slice.into_iter().zip(&alias_common.lambda_set_variables);
                ...
                let type_argument_abilities = Slice::extend_new(
                    ...
                    alias_common
                        .type_arguments
                        .iter()
                        .map(|a| a.value.opt_ability),
                );

                let alias_shared = AliasShared {
                    symbol: alias_common.symbol,
                    type_argument_abilities,
                    type_argument_regions,
                    lambda_set_variables: lambda_set_slice,
                };
            ...
            Type::Alias {
                ...
                let alias_shared =
                    self.alias_shared_help(*symbol, type_arguments, lambda_set_variables);
